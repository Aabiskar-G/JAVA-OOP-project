...
int main(int argc, char* argv[]) {
    ...
    // -------------------------------------------------------------------------
    // Addition with seconds (new test)
    vector <<= 1;
    int seconds = 600;
    time = Time{9, 30, 0}; // Reusing the 'time' variable
    if(fails(time + seconds, "09:40:00")) result |= vector;
    if(fails(seconds + time, "09:40:00")) result |= vector;

    // Print specific output to confirm functionality
