...
public class WordSearch {
    ...
    public void solve() {
        ...
        int extraPuzzles = NUM_PUZZLES % NUM_THREADS;

        int currentPuzzle = 0;
        for (int i = 0; i < NUM_THREADS; i++) {
            int firstPuzzle = currentPuzzle;
            int lastPuzzlePlusOne = firstPuzzle + puzzlesPerThread;
            if (extraPuzzles > 0) {
                lastPuzzlePlusOne++;
                extraPuzzles--;
            }
            currentPuzzle = lastPuzzlePlusOne;

            final int start = firstPuzzle;
            final int end = lastPuzzlePlusOne;
            final int finalI = i;

            threads[i] = new Thread(() -> {
                solve(finalI, start, end);
            });
            threads[i].start();
        }
        ...
        for (Thread thread : threads) {
            try {
                ...
                System.err.println("Error: One of the threads was interrupted.");
            }
        }
    }
